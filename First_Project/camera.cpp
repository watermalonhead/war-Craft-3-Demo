...
void Camera::CameraUpdate(float timeDelta)
{
	timeDelta *= 3;
	long ClientWidth = GetSystemMetrics(SM_CXSCREEN);
	long ClientHeight = GetSystemMetrics(SM_CYSCREEN);
	POINT CurrentPos;
	GetCursorPos(&CurrentPos);
	ClientToScreen(g_hWnd,&CurrentPos); 
	if (CurrentPos.x>ClientWidth-100)
	{
		long DeltaX = CurrentPos.x - ClientWidth-100;
		TheCamera.strafe(-DeltaX*timeDelta);
	}
	if (CurrentPos.x<100)
	{
		long DeltaX = CurrentPos.x - 100;
		TheCamera.strafe(DeltaX*timeDelta);
	}
	if (CurrentPos.y<100)
	{
		long DeltaY = CurrentPos.y - 100;
		TheCamera.walk(-DeltaY*timeDelta);
	}
	if (CurrentPos.y>ClientHeight-100)
	{
		long DeltaY = CurrentPos.y - ClientHeight - 100;
		TheCamera.walk(DeltaY*timeDelta);
	}
}
